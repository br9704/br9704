...
/* ── Background: the portfolio dither, applied PARTIALLY (faded, not everywhere) ── */
// cover-resize the dither, then mask it with a soft radial so it reads as texture
// behind the icons and fades to clean black toward the edges + bottom.
const dither = await sharp("dl/dither.png").resize(W, H, { fit: "cover", position: "top" }).png().toBuffer();
const mask = Buffer.from(
  `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
     <defs><radialGradient id="m" cx="50%" cy="30%" r="62%">
...
/* ── 3 flagship icons (white) on thin B&W tiles, centered near the top ── */
...
const IP = 112 * S, GAP = 52 * S, GLYPH = 74 * S;
const rowW = ICONS.length * IP + (ICONS.length - 1) * GAP;
const startX = Math.round((W - rowW) / 2), rowY = 40 * S;
...
for (let i = 0; i < ICONS.length; i++) {
  ...
  lab += `<text x="${x + IP / 2}" y="${rowY + IP + 26 * S}" text-anchor="middle" font-family="${MONO}" font-size="${15 * S}" fill="#cfcfcf">${names[ICONS[i]]}</text>`;
}
lab += `</svg>`;
comps.push({ input: Buffer.from(lab), left: 0, top: 0 });

/* ── text: tagline + a small footer line (no name, no cat) ── */
const textSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
  <text x="${W / 2}" y="${266 * S}" text-anchor="middle" font-family="${MONO}" font-weight="700" font-size="${40 * S}" fill="#ffffff" letter-spacing="${1 * S}">building great experiences</text>
  <text x="${W / 2}" y="${306 * S}" text-anchor="middle" font-family="${MONO}" font-size="${17 * S}" fill="#bdbdbd">apps · SaaS · agentic AI   —   iOS · web · design + engineering as one</text>
  <text x="${W / 2}" y="${344 * S}" text-anchor="middle" font-family="${MONO}" font-size="${15 * S}" fill="#8f8f8f">melbourne, au   ·   open to work   ·   brunojaamaa.dev</text>
</svg>`;
comps.push({ input: Buffer.from(textSvg), left: 0, top: 0 });

await sharp(base).composite(comps).png().toFile("assets/banner.png");
console.log("dither banner (no cat) written");
